...
function sequenceGenerator (props) {
	//
	this.schema = {};
	this.props = props;
	//
	...
	//types
	this.Linker = Linker;
	this.RandomValueFromList = RandomValueFromList;
	this.UniqueRandomValueFromList = UniqueRandomValueFromList;
	...
	function RandomStringType (alphabet = "") {
		...
		return (props = {}) => {
			this.props = Props;
			const len =  eq("function", props.len) ? props.len() : (this.props.default_length || config.default_length || 65536);
			const need = eq("function", props.need) ? props.need() : (this.props.needs || config.needs || false);
			const chance = eq("function", props.chance) ? props.chance() : (this.props.chance || config.chance || 0.5);
			tq("number", len);
			tq("boolean", need);
			tq("number", chance);
			//
	...
	function RandomNumberType () {
		...
		return (props = {}) => {
			this.props = Props;
			const max = eq("function", props.max) ? props.max() : (this.props.maxNumber || config.maxNumber || 65536);
			const min =  eq("function", props.min) ? props.min() : (this.props.minNumber || config.minNumber || 0);
			const int = eq("function", props.int) ? props.int() : (this.props.integer || config.integer || true);
			const need = eq("function", props.need) ? props.need() : (this.props.needs || config.needs || false);
			const chance = eq("function", props.chance) ? props.chance() : (this.props.chance || config.chance || 0.5);
			tq("number", max);
			tq("number", min);
	...
	function RandomBooleanType () {
		...
		return (props = {}) => {
			this.props = Props;
			const prob = eq("function", props.prob) ? props.prob() : (this.props.probability || config.probability || 0.5);
			const need = eq("function", props.need) ? props.need() : (this.props.needs || config.needs || false);
			const chance = eq("function", props.chance) ? props.chance() : (this.props.chance || config.chance || 0.5);
			tq("number", prob);
			tq("boolean", need);
	...
	function MakeFunc (func, schema = {}) {
		//alphabet
		this.props = Props;
		const dec = this.props.alphabet || config.alphabet || "0123456789abcdef";
		const n = func.name;
		switch (n) {
